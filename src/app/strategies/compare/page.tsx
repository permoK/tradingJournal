...
export default function StrategyComparison() {
  ...
  const handleCompare = async () => {
    ...
    try {
      console.log('Starting strategy comparison with:', { selectedStrategies, userId: user?.id });
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || `HTTP ${response.status}: Failed to fetch comparison data`);
      }
      ...
      setError(err.message || 'An unexpected error occurred while comparing strategies');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <AppLayout>
        ...
        <div className="bg-white p-8 rounded-lg shadow-sm border border-slate-200 text-center">
          <div className="mb-4">
            <FiBarChart2 className="h-12 w-12 text-slate-400 mx-auto mb-4" />
            <h3 className="text-lg font-semibold text-slate-900 mb-2">
              {strategies?.length === 0 ? 'No Strategies Found' : 'Need More Strategies'}
            </h3>
            <p className="text-slate-600 mb-6">
              {strategies?.length === 0
                ? 'You need to create strategies before you can compare them.'
                : 'You need at least 2 strategies to perform a comparison.'
              }
            </p>
          </div>
          <Link
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <p className={`text-sm font-medium ${getSuccessRateColor(strategy.success_rate || 0)}`}>
      ...
      {comparisonData && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {comparisonData.summary.bestPerformer && (
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className={`text-sm ${getProfitLossColor(comparisonData.summary.bestPerformer.metrics.netProfitLoss)}`}>
                      ...
                      {(comparisonData.summary.bestPerformer.metrics.netProfitLoss || 0).toFixed(2)}
                    </p>
                  </div>
            ...
            {comparisonData.summary.mostConsistent && (
              <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className={`text-sm ${getSuccessRateColor(comparisonData.summary.mostConsistent.metrics.successRate)}`}>
                      {(comparisonData.summary.mostConsistent.metrics.successRate || 0).toFixed(1)}%
                    </p>
                  </div>
            ...
            {comparisonData.summary.mostActive && (
              <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-slate-600">
                      {comparisonData.summary.mostActive.metrics.totalTrades || 0} trades
                    </p>
                  </div>
            ...
            {comparisonData.summary.bestProfitFactor && (
              <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200">
                <div className="flex items-center justify-between">
                  <div>
